...
class DatabaseManager:
    ...
    def _table_info(self) -> None:
        ...
        for table in tables:
            ...
            logger.info(f'Table: {table}. Number of rows: {count['cnt'][0]}')
    ...
    def full_load(self, table_name: str, df: pd.DataFrame) -> None:
        ...
        logger.info(f'Full loaded {len(df)} rows to table {table_name}')
    ...
    def append(self, table_name: str, df: pd.DataFrame, id_col: str) -> None:
        ...
        logger.info(f'Deduplicated {len(df) - len(df_to_append)} rows by {id_col}')
        df_to_append.to_sql(name=table_name, con=self.engine, if_exists='append', index=False)
        logger.info(f'Appended {len(df_to_append)} rows to table {table_name}')
